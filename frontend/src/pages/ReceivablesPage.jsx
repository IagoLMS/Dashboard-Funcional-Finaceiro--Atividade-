...
import {
  ...
  Clock, Download, FileText, Filter, Pencil, Plus,
  RefreshCw, TrendingDown, TrendingUp, X, XCircle,
} from 'lucide-react';
import { computeReceivableStatus, RECEIVABLE_CLIENTS } from '../utils/data';
...
export default function ReceivablesPage() {
  const { data: list, create, update: updateReceivable, remove, registerPayment } = useReceivables();
  ...
  const handleEdit = async (data) => {
    await updateReceivable(active.id, data);
    closeModal();
  };

  const handlePay = async (data) => {
    const result = await registerPayment(active.id, data);
