...
import { KPI_DATA as STATIC_KPI, DEPARTMENTS as STATIC_DEPTS } from './data';

/**
 * Exports the financial dashboard PDF.
 * @param {{ kpis?: object, departments?: Array }} [opts] — pass real data when in api mode
 */
export function exportPDF({ kpis: kpiData, departments: deptData } = {}) {
  const KPI_DATA    = kpiData  || STATIC_KPI;
  const DEPARTMENTS = deptData || STATIC_DEPTS;
