...
import {
  ...
  getCostCenterAnalysis, getCostCentersMonthlyVariation,
  COST_CENTER_TYPES, PAYABLE_CATEGORIES,
} from '../utils/data';
...
export default function CostCentersPage() {
  ...
  useEffect(() => { setCenters(getCostCenters()); }, []);
  ...
  const enriched = useMemo(() =>
    centers.map(c => {
      const a = getCostCenterAnalysis(c.id) || { currentCost: 0, paidCost: 0, percentage: 0, status: 'within' };
      return {
        ...c,
  ...
  const variation   = useMemo(() => getCostCentersMonthlyVariation(), [centers]);
  ...
  const handleSave = async () => {
    ...
    const result = modal === 'create'
      ...
      : updateCostCenter(active.id, payload);

    setSaving(false);

    if(result.error) { setFormErr(result.error); return; }
    setCenters(result.updated);
    closeModal();
  };
  ...
  const handleDelete = async () => {
    ...
    const result = deleteCostCenter(active.id);
    setSaving(false);
  ...
  // Detail analysis (recomputed when active changes)
  const analysis = useMemo(
    () => active ? getCostCenterAnalysis(active.id) : null,
    [active, centers]
  );
