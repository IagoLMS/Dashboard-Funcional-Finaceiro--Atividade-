...
import {
  registerPayablePayment, computePayableStatus,
  getPayables, createPayable, updatePayable, deletePayable,
  PAYABLE_SUPPLIERS, PAYABLE_CATEGORIES, PAYABLE_ORIGINS,
} from '../utils/data';
...
export default function PayablesPage() {
  ...
  useEffect(() => { setPayables(getPayables()); }, []);
  ...
  const handleSave = async () => {
    ...
    if(modal === 'create') {
      ...
      const updated = updatePayable(active.id, {
        supplier:    form.supplier,
        origin:      form.origin,
        category:    form.category,
        totalValue:  Number(form.totalValue),
  ...
  const handleRegisterPayment = async () => {
    ...
    const { updated, error } = registerPayablePayment(active.id, {
      amount: amt,
      date:   payForm.date,
  ...
  const handleCancel = async () => {
    ...
    const updated = deletePayable(active.id);
    setPayables(updated);
    setSaving(false);
    closeModal();
  };
